...
class ReportsServiceUnitTest {
    ...
    void setUp() {
        ...
        lenient().when(countryStrategy.getGroupingType()).thenReturn(ReportGrouping.COUNTRY);
    ...
    @Test
    void getReport_Fail_StrategyNotFound() {


        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () ->
                reportsService.getReport(ReportGrouping.BROKER, null, null, null, null, null)
        );

        assertTrue(exception.getMessage().contains("No strategy found for grouping"));
    }
}